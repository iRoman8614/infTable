...
//
//                 const currentScrollTop = containerRef.current.scrollTop;
//                 const currentFirstVisibleIndex = Math.floor(currentScrollTop / dynamicRowHeight); // Используем динамическую высоту
...
//                             setScrollTop(compensatedScrollTop);
//
//                             setTimeout(() => {isScrollCompensating.current = false;}, 50);
//                         }
//                     });
//
//                     return updatedDates;
...
//             }, 150);
//         }
//     }, [dates, scrollBatchSize, dynamicRowHeight, loadBatch, scheduleRowspanRecalculation]);
//
//     const handleScrollImmediate = useCallback(async () => {
//         if (!containerRef.current || isScrollCompensating.current) return;
//
...
//             await Promise.allSettled(promises);
//         }
//     }, [dates, extendDates, dynamicRowHeight, bufferSize, processPendingHeaderUpdate]);
//
//     const handleScrollThrottled = useMemo(
//         () => smartThrottle(handleScrollImmediate, 8),
//         [handleScrollImmediate]
...
//
//                     const containerHeight = containerRef.current.clientHeight;
//                     const visibleRows = Math.ceil(containerHeight / dynamicRowHeight); // Используем динамическую высоту
//                     const totalRowsNeeded = visibleRows + (bufferSize * 2);
//                     const initialBatchSize = Math.max(scrollBatchSize * 2, totalRowsNeeded);
//
//                     const startIndex = Math.max(0, todayIndex - Math.floor(initialBatchSize / 2));
...
//                     setTimeout(() => {
//                         if (containerRef.current) {
//                             const targetScroll = todayIndex * dynamicRowHeight - (containerHeight / 2) + (dynamicRowHeight / 2); // Используем динамическую высоту
//                             containerRef.current.scrollTop = Math.max(0, targetScroll);
//
//                             setContainerHeight(containerHeight);
//                             setScrollTop(containerRef.current.scrollTop);
...
//             initializeTable();
//         }
//     }, [isInitialized, dates.length, generateInitialDates, today, dynamicRowHeight, scrollBatchSize, loadBatch, bufferSize, scheduleRowspanRecalculation]); // Обновить зависимости
//
//     useEffect(() => {
//         if (isInitialized) {
//             loadVisibleData();
...
//                                 </div>
//
//                                 {expandedGroups.has(groupName) && renderHeaderStructure[groupName] && (
//                                     <div>
//                                         {renderHeaderStructure[groupName].map(element => (
//                                             <div key={element}>
//                                                 <div style={{
