...
//                 const batchData = await activeDataProvider(startDate, direction, batchSize);
//
//                 // Проверяем формат данных - поддерживаем оба варианта
//                 let dataArray;
//                 if (batchData && batchData["data"] && Array.isArray(batchData["data"])) {
//                     // Новый формат со строковым ключом "data"
//                     dataArray = batchData["data"];
//                 } else if (batchData && batchData.data && Array.isArray(batchData.data)) {
//                     // Старый формат с обычным свойством data
//                     dataArray = batchData.data;
//                 } else {
...
//                 const dataForTransform = { data: dataArray };
...
export const useTableLogic = ({
    ...
    const loadBatch = useCallback(async (startDate, direction, batchSize) => {
        ...
        fetchingPromises.current[batchKey] = promise;
        return promise;
    }, [loadingBatches, dataProvider, onDataLoad, onError, createRangeKey, isRangeLoaded]);
    ...
    const extendDates = useCallback(async (direction, isPreemptive = false) => {
        ...
        await Promise.allSettled(loadPromises);
        scheduleRowspanRecalculation();
    }, [dates, scrollBatchSize, rowHeight, loadBatch, scheduleRowspanRecalculation, isRangeLoaded]);
